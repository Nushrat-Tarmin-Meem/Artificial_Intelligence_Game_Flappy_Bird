...
space_sound = pygame.mixer.Sound('.\\sounds\\space.wav')
...
def reset_game():
    ...
    total_score = 0
    fall_played = False
    return score, coin_count ,total_score,fall_played
...
mmbutton = Button(screen_width // 2 - 50, screen_height // 2 - 100, button_img)
...
while run:
    ...
    if game_over == False and flying == True:
        ...
        pipe_group.update()
        coin_group.update()
        WINNING_TIME = 20  # 60 seconds
        start_time = pygame.time.get_ticks() 

        current_time = (pygame.time.get_ticks() - start_time) / 1000
        if current_time >= WINNING_TIME:
            draw_text("Score and Survival Achieved! You won!", font, blue, int(screen_width / 2.5), screen_height / 2)
            game_over = True
    ...
    if game_over == False and score == 5:
        draw_text("Great job!", font, blue, int(screen_width / 2.5), 70)

    if game_over == False and score == 10:
        draw_text("Keep going!", font, blue, int(screen_width / 2.5), 70)

    if game_over == False and score == 15:
        draw_text("Flying masterfully!", font, blue, int(screen_width / 2.5), 70)
    ...
    if game_over == True or total_score == 20:
        if total_score == 20:
            win = True
            if(tada_played == False):
                tada_sound.play()
                tada_played = True
            draw_text("You Won", font, red, int(screen_width / 2.6), screen_height / 2)
            boost_button.hide()
            if button.draw() == True:
                scroll_speed = 0
                game_over = False
                score, coin_count,total_score = reset_game()

        else :
            if(fall_played == False):
                fall_sound.play()
                fall_played = True
            draw_text("Game Over", font, red, int(screen_width / 2.6), screen_height / 2)
            boost_button.hide()
            if button.draw() == True:
                scroll_speed = 0
                game_over = False
                score, coin_count,total_score,fall_played = reset_game()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and flying == False and game_over == False:
            flying = True
        if event.type == pygame.MOUSEBUTTONDOWN and win == True and game_over == False:
            flying = False
            if mmbutton.draw() == True:
                scroll_speed = 0
                game_over = False
                score, coin_count,total_score,fall_played = reset_game()
